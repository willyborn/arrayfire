...
template<typename T>
void memcopy(const Param out, const Param in, const dim_t indims) {
    dims_t idims_{
    ...
    if (elements > 0) {
        if (isLinear) {
            ...
            getQueue().enqueueCopyBuffer(
                *in.data, *out.data, in.info.offset * sizeof(T),
                out.info.offset * sizeof(T), elements * sizeof(T), nullptr,
                nullptr);
        } else {
            ...
            memCopy(cl::EnqueueArgs(getQueue(), global, local), *out.data,
                    ostrides_, static_cast<unsigned>(out.info.offset), *in.data,
                    idims_, istrides_, static_cast<unsigned>(in.info.offset));
...
struct ParamOffset {
    Param p;        // Param of the input buffer
    dim_t ooffset;  // offset in the output buffer
};

template<typename T>
void memcopy(const Param out, const std::vector<struct ParamOffset> &ins) {
    bool kernelLoaded = false;
    Kernel memCopy;
    const int ondims = out.info.dims[3] > 1   ? 4
                       : out.info.dims[2] > 1 ? 3
                       : out.info.dims[1] > 1 ? 2
                       : out.info.dims[0] > 0 ? 1
                                              : 0;
    ...
    for (auto &in : ins) {
        dims_t idims_{static_cast<int>(in.p.info.dims[0]),
                      static_cast<int>(in.p.info.dims[1]),
                      static_cast<int>(in.p.info.dims[2]),
                      static_cast<int>(in.p.info.dims[3])};
        dims_t istrides_{static_cast<int>(in.p.info.strides[0]),
                         static_cast<int>(in.p.info.strides[1]),
                         static_cast<int>(in.p.info.strides[2]),
                         static_cast<int>(in.p.info.strides[3])};
        dims_t ostrides_{static_cast<int>(out.info.strides[0]),
                         static_cast<int>(out.info.strides[1]),
                         static_cast<int>(out.info.strides[2]),
                         static_cast<int>(out.info.strides[3])};
        int indims_ = ondims;
        // We include odims_ each time in the check, in the hope that we
        // find more Linear sections due to the different ndims_.
        bool isLinear = true;
        int elements  = (indims_ == 0) ? 0 : 1;
        for (int dim = 0; dim < indims_; ++dim) {
            isLinear &= (elements == istrides_.dims[dim]) &
        ...
        if (elements > 0) {
            if (isLinear) {
                getQueue().enqueueCopyBuffer(
                    *in.p.data, *out.data, in.p.info.offset * sizeof(T),
                ...
                for (int c = 0; c < indims_; ++c) {
                    if (idims_.dims[c] == 1) {
                        // Eliminate the columns with 1, so that we have
                        // more appropriate dimensions in the local & global
                        // parameters
                        for (int i = c; i < indims_ - 1; ++i) {
                            idims_.dims[i]    = idims_.dims[i + 1];
                            istrides_.dims[i] = istrides_.dims[i + 1];
                        ...
                        --indims_;
                        idims_.dims[indims_] = 1;
                        --c;  // Redo this column, since it is eliminated
                              // now!!
                    }
                }
                // Increase work inside each thread (if last dim is free and a
                ...
                // This kernel is memory bound, so focus on caching
                // (dim0+++)
                const int dim0 = (idims_.dims[0] <= 128) ? 128 : 256;
                const int dim1 =
                ...
                const cl::NDRange global(dim0 * divup(idims_.dims[0], dim0),
                                         ...
                                         dim2 * divup(idims_.dims[2], dim2));
                memCopy(cl::EnqueueArgs(getQueue(), global, local), *out.data,
                        ostrides_, static_cast<unsigned>(in.ooffset),
                        *in.p.data, idims_, istrides_,
                        static_cast<unsigned>(in.p.info.offset));
                CL_DEBUG_FINISH(getQueue());
            }
        }
    }
