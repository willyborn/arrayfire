...
void fast(const unsigned arc_length, unsigned *out_feat, Param &x_out,
    ...
    getQueue().enqueueFillBuffer<float>(*d_score, 0.0, 0, in.info.dims[0] * in.info.dims[1] * sizeof(float));
	...
	getQueue().enqueueFillBuffer<unsigned>(*d_total, 0, 0, sizeof(unsigned));
