...
template<typename T>
void join(Param out, int jdim, Param in1, Param in2) {
    ...
    auto join2 = common::getKernel("join2", {join_cl_src}, targs, options);
    ...
    for (int c = 0; c < ondims; ++c) {
        if (out.info.dims[c] == 1) {
            ...
            for (int i = c; i < ondims - 1; ++i) {
                ...
                in1.info.dims[i]    = in1.info.dims[i + 1];
                in1.info.strides[i] = in1.info.strides[i + 1];
                in2.info.dims[i]    = in2.info.dims[i + 1];
                in2.info.strides[i] = in2.info.strides[i + 1];
            }
            --ondims;
            if (c <= jdim) --jdim;
            out.info.dims[ondims] = 1;
            in1.info.dims[ondims] = 1;
            in2.info.dims[ondims] = 1;
            --c;  // Redo this column, since it is eliminated now!!
        }
    }
    dim4 info(ondims, in1.info.dims);
    info.dims[jdim] = std::max(info.dims[jdim], in2.info.dims[jdim]);
    const int dim0  = (info.dims[0] == 1)     ? 1
                      : (info.dims[0] <= 128) ? 128
                                              : 256;
    const int dim1  = (dim0 == 1) ? (info.dims[1] <= 128) ? 128 : 256
                      : (dim0 == 256) || (info.dims[1] & 0x1) ? 1
                                                              : 2;
    const int dim2  = (dim0 * dim1 == 256) || (info.dims[2] & 0x1) ? 1 : 2;
    const cl::NDRange local(dim0, dim1, dim2);
    const cl::NDRange global(dim0 * divup(out.info.dims[0], dim0),
                             dim1 * divup(out.info.dims[1], dim1),
                             dim2 * divup(out.info.dims[2], dim2));
    join2(cl::EnqueueArgs(getQueue(), global, local), *out.data, out.info, jdim,
          *in1.data, in1.info, *in2.data, in2.info);
...
void join(Param out, int jdim, std::vector<Param> &in) {
    ...
    switch (in.size()) {
        ...
        case 1:
            return kernel::memcopy<T>(
                *out.data, dim4(4, out.info.strides), *in[0].data,
                dim4(ondims, in[0].info.dims), dim4(4, in[0].info.strides),
                in[0].info.offset, ondims, out.info.offset);
        case 2: return join<T>(out, jdim, in[0], in[1]);
    }
    ...
    for (int start = 0, end = std::min(static_cast<int>(in.size()), start + 5);
            ...
            case 1: {
                ...
                return kernel::memcopy<T>(
                    *out.data, dim4(4, out.info.strides), *i.data,
                    dim4(ondims, i.info.dims), dim4(4, i.info.strides),
                    i.info.offset, ondims, out.info.offset);
            }
            case 2: return join<T>(out, jdim, in[start], in[start + 1]);
        }
        ...
        const int dim1 = (dim0 == 1) ? (info.dims[1] <= 128) ? 128 : 256
                         : (dim0 == 256) || (info.dims[1] & 0x1) ? 1
                                                                 : 2;
        const cl::NDRange local(dim0, dim1, 1);
        const cl::NDRange global(dim0 * divup(info.dims[0], dim0),
                                 dim1 * divup(info.dims[1], dim1), size);

        switch (size) {
            case 3:
                joinN(cl::EnqueueArgs(getQueue(), global, local), *out.data,
                      out.info, jdim, *in[start].data, in[start].info,
