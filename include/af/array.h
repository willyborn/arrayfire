...
#undef ASSIGN
    ...
    */
     ...
     */
    explicit array(af_array& handle);
    ...
    */
    explicit array(dim_t dim0, dtype ty = f32);
    ...
    */
    explicit array(dim_t dim0, dim_t dim1, dtype ty = f32);
    ...
    */
    explicit array(dim_t dim0, dim_t dim1, dim_t dim2, dtype ty = f32);
    ...
    */
    explicit array(dim_t dim0, dim_t dim1, dim_t dim2, dim_t dim3,
                   dtype ty = f32);
    ...
    /**
        ...
        \param[in] ty optional label describing the data type
                   (default is f32)
    */
    explicit array(const dim4& dims, dtype ty = f32);
    ...
    */
    template<typename T>
    explicit array(dim_t dim0, const T* pointer, af::source src = afHost);
    ...
    */
    template<typename T>
    explicit array(dim_t dim0, dim_t dim1, const T* pointer,
    ...
    */
    template<typename T>
    explicit array(dim_t dim0, dim_t dim1, dim_t dim2, const T* pointer,
                   af::source src = afHost);
    ...
    */
    template<typename T>
    explicit array(const dim4& dims, const T* pointer, af::source src = afHost);
    ...
    array(std::initializer_list<T> list) : arr(nullptr) {
        dim_t size = list.size();
        if (af_err __aferr = af_create_array(
                &arr, list.begin(), 1, &size,
    ...
    template<typename T, typename = typename std::enable_if<
                             std::is_fundamental<T>::value, void>::type>
    array(const af::dim4& dims, std::initializer_list<T> list) : arr(nullptr) {
    ...
    */
    ...
    /**
       ...
       \param[in] dim3 fourth dimension
    */
    array(const array& input, const dim_t dim0, const dim_t dim1 = 1,
    ...
    */
    ...
    /**
       \brief Get the device pointer from the array and lock the buffer in
       memory manager.

       The device memory returned by this function is not freed until unlock()
       is called.

       /note When using the OpenCL backend and using the cl_mem template
       argument, the delete function should be called on the pointer returned by
       this function.
    */
    template<typename T> T* device() const;
    ...
    */
    ...
    /**
        ...
        \ingroup array_mem_operator_paren
    */
    array::array_proxy operator()(const index& s0, const index& s1,
                                  const index& s2 = span,
                                  const index& s3 = span);

    /**
        \copydoc operator()(const index &, const index &, const index &, const
       index &)

        \ingroup array_mem_operator_paren
    */
    const array::array_proxy operator()(const index& s0, const index& s1,
                                        const index& s2 = span,
    ...
    /// x   | | b8      | x   | x   | x   | x   | x   | x   | x  | x  | x   | x
    /// | x   | x   | x   | | s64     | x   | x   | x   | x   |     |     |    |
    /// | x   | x   |     |     | x   | | u64     | x   | x   | x   | x   | | |
    /// |    | x   | x   |     |     | x   | | s16     | x   | x   | x   | x   |
    /// x   | x   |    |    | x   | x   | x   | x   | x   | | u16     | x   | x
    /// | x   | x   | x   | x   |    |    | x   | x   | x   | x   | x   | | f16
    /// | x   | x   | x   | x   |     |     |    |    |     |     |     |     |
    /// x   | If you want to avoid this behavior use af_eval after the first
    /// cast operation. This will ensure that the cast operation is performed on
    /// the af::array
    ///
...
#define ASSIGN_(OP2)                                                        \
    ...
    array& OP2(                                                             \
        const unsigned char& val); /**< \copydoc OP2##(const array &) */    \
    array& OP2(const long& val);   /**< \copydoc OP2##(const array &) */    \
    array& OP2(                                                             \
        const unsigned long& val);    /**< \copydoc OP2##(const array &) */ \
...
#undef ASSIGN_
    ...
    ///
    /// This method can be called to take control of the device pointer from the
    /// memory manager. While a buffer is locked, the memory manager doesn't
    /// free the memory until unlock() is invoked.
    void lock() const;
    ...
    ///
    /// An array can be locked by the user by calling `arry.lock` or
    /// `arr.device` or `getRawPtr` function.
    bool isLocked() const;
#endif
    ...
    /// Calling this method gives back the control of the device pointer to the
    /// memory manager.
    void unlock() const;
};
// end of class array
...
#define BIN_OP_(OP)                                                            \
    ...
    AFAPI array OP(                                                            \
        const bool& lhs,                                                       \
        const array& rhs); /**< \copydoc OP##(const array&, const array&) */   \
    AFAPI array OP(                                                            \
        const int& lhs,                                                        \
        const array& rhs); /**< \copydoc OP##(const array&, const array&) */   \
    AFAPI array OP(                                                            \
    ...
    AFAPI array OP(                                                            \
        const array& lhs,                                                      \
        const bool& rhs); /**< \copydoc OP##(const array&, const array&) */    \
    AFAPI array OP(                                                            \
        const array& lhs,                                                      \
        const int& rhs); /**< \copydoc OP##(const array&, const array&) */     \
    AFAPI array                                                                \
    OP(const array& lhs,                                                       \
       const unsigned& rhs); /**< \copydoc OP##(const array&, const array&) */ \
    AFAPI array OP(                                                            \
        const array& lhs,                                                      \
        const char& rhs); /**< \copydoc OP##(const array&, const array&) */    \
    AFAPI array OP(const array& lhs,                                           \
                   const unsigned char&                                        \
                       rhs); /**< \copydoc OP##(const array&, const array&) */ \
    AFAPI array OP(                                                            \
        const array& lhs,                                                      \
        const long& rhs); /**< \copydoc OP##(const array&, const array&) */    \
    AFAPI array OP(const array& lhs,                                           \
                   const unsigned long&                                        \
                       rhs); /**< \copydoc OP##(const array&, const array&) */ \
    AFAPI array OP(const array& lhs,                                           \
    ...
    AFAPI array OP(                                                            \
        const array& lhs,                                                      \
        const double& rhs); /**< \copydoc OP##(const array&, const array&) */  \
    AFAPI array OP(                                                            \
        const array& lhs,                                                      \
        const float& rhs); /**< \copydoc OP##(const array&, const array&) */   \
    AFAPI array OP(                                                            \
        const array& lhs,                                                      \
        const cfloat& rhs); /**< \copydoc OP##(const array&, const array&) */  \
...
#define BIN_OP(OP)                                                           \
    ...
    AFAPI array OP(                                                          \
        const array& lhs,                                                    \
        const short& rhs); /**< \copydoc OP##(const array&, const array&) */ \
...
/// \param[in] rhs the right hand side value of the operand
///
/// \returns an array of type b8 with the equality operation performed on each
/// element
BIN_OP(operator==)
/// @}
...
/// @{
/// \brief Performs an inequality operation on two arrays or an array and a
/// value.
///
/// \param[in] lhs the left hand side value of the operand
/// \param[in] rhs the right hand side value of the operand
///
/// \returns    an array of type b8 with the != operation performed on each
/// element
///             of \p lhs and \p rhs
BIN_OP(operator!=)
/// @}
...
/// @{
/// \brief Performs a lower than operation on two arrays or an array and a
/// value.
///
/// \param[in] lhs the left hand side value of the operand
/// \param[in] rhs the right hand side value of the operand
...
/// @{
/// \brief Performs an lower or equal operation on two arrays or an array and a
/// value.
///
/// \param[in] lhs the left hand side value of the operand
/// \param[in] rhs the right hand side value of the operand
...
/// @{
/// \brief Performs an greater than operation on two arrays or an array and a
/// value.
///
/// \param[in] lhs the left hand side value of the operand
/// \param[in] rhs the right hand side value of the operand
...
/// @{
/// \brief Performs an greater or equal operation on two arrays or an array and
/// a value.
///
/// \param[in] lhs the left hand side value of the operand
/// \param[in] rhs the right hand side value of the operand
...
/// \param[in] rhs the right hand side value of the operand
///
/// \returns    an array of type b8 with a logical OR operation performed on
/// each
...
/// \param[in] rhs the right hand side value of the operand
///
/// \returns    an array of type b8 with a logical AND operation performed on
/// each
///             element of \p lhs and \p rhs
AFAPI array operator&&(const array& lhs, const array& rhs);
AFAPI array operator&&(const array& lhs, const bool& rhs);
...
/// Evaluate an expression (nonblocking).
inline const array& eval(const array& a) {
    a.eval();
    return a;
}

#if AF_COMPILER_CXX_VARIADIC_TEMPLATES
template<typename... ARRAYS> inline void eval(ARRAYS... in) {
...
inline void eval(const array& a, const array& b, const array& c, const array& d,
                 const array& e) {
...
/**
   ...
   This function will create an \ref af_array handle from the memory provided in
   \p data.
   ...
   \param[in]   dims A C pointer with \p ndims elements. Each value represents
   the size of that dimension \param[in]   type The type of the \ref af_array
   object

   \returns \ref AF_SUCCESS if the operation was a success
*/
AFAPI af_err af_create_array(af_array* arr, const void* const data,
...
/**
   ...
   \returns \ref AF_SUCCESS if the operation was a success
*/
AFAPI af_err af_create_handle(af_array* arr, const unsigned ndims,
...
*/
AFAPI af_err af_write_array(af_array arr, const void* data, const size_t bytes,
                            af_source src);
...
/**
    ...
    \param[out] d0 is the output that contains the size of first dimension of \p
   arr \param[out] d1 is the output that contains the size of second dimension
   of \p arr \param[out] d2 is the output that contains the size of third
   dimension of \p arr \param[out] d3 is the output that contains the size of
   fourth dimension of \p arr \param[in] arr is the input array
...
/**
    ...
    \param[out] result is true if arr is of type \ref c32 or \ref c64, otherwise
   false \param[in] arr is the input array
...
/**
    ...
    \param[out] result is true if arr is NOT \ref c32 or \ref c64, otherwise
   false \param[in] arr is the input array
...
/**
    ...
    \param[out] result is true if arr is of type \ref f64 or \ref c64, otherwise
   false \param[in] arr is the input array
...
/**
    ...
    \param[out] result is true if arr is of type \ref f32 or \ref c32, otherwise
   false \param[in] arr is the input array
...
/**
    ...
    \param[out] result is true if arr is of type \ref f32 or \ref f64, otherwise
   false \param[in] arr is the input array
