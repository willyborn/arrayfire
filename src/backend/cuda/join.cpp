...
Array<T> join(const int jdim, const Array<T> &first, const Array<T> &second) {
    ...
    if (first.isReady()) {
        ...
        vector<Param<T>> outputs{
            {out.getData().get(), fdims.dims, out.strides().dims}};
    ...
    if (second.isReady()) {
        ...
        vector<Param<T>> outputs{
            {out.getData().get() + fdims.dims[jdim] * out.strides().dims[jdim],
...
Array<T> join(const int jdim, const std::vector<Array<T>> &inputs) {
    ...
    for (auto &iArray : inputs) {
        if (iArray.isReady()) {
            ...
            vector<Param<T>> outputs{{out.getData().get() + outOffset,
                                      iArray.dims().dims, out.strides().dims}};
