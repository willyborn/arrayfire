...
void join(Param out, int jdim, Param in1, Param in2) {
    ...
    const int dim0 =
        (in1.info.dims[0] <= 128 && in2.info.dims[0] <= 128) ? 128 : 256;
    const int dim1 =
        (dim0 == 256) || (in1.info.dims[1] & 0x1) && (in2.info.dims[1] & 0x1)
            ? 1
            : 2;
    const int dim2 = (dim0 * dim1 == 256) ||
                             (in1.info.dims[2] & 0x1 && in2.info.dims[2] & 0x1)
                         ? 1
                         : 2;
