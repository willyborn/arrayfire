...
Array<T> join(const int jdim, const Array<T> &first, const Array<T> &second) {
    ...
    const vector<struct kernel::ParamOffset> ins{
        {first, 0}, {second, fdims.dims[jdim] * out.strides().dims[jdim]}};
    kernel::memcopy<T>(out, ins);
...
Array<T> join(const int jdim, const vector<Array<T>> &inputs) {
    ...
    Array<T> out = createEmptyArray<T>(odims);
    vector<kernel::ParamOffset> ins;
    ins.reserve(inputs.size());
    {
        dim_t d         = 0;
        const dim_t tmp = out.strides().dims[jdim];
        for (auto &iArray : inputs) {
            ins.push_back({iArray, d * tmp});
