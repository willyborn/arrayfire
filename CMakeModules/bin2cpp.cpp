...
stringstream removeComments(ifstream &input, string &filename) {
  stringstream ss;
  char line[256]{0}; // Maximum length of lines in OpenCL code is limited to 256
  ...
  while (input.getline(line, sizeof(line) - 1)) {
    ...
    do {
      ...
      while (token) {
        ...
        case MULTILINE:
          if (subtoken = strstr(token, tokenCommentsEnd)) {
            if (del.start == nullptr)
              del.start = line;
            del.end = subtoken + strlen(tokenCommentsEnd) - local + line;
            dels.push_back(del);
            del = {nullptr, nullptr};
          ...
          break;
        case STRING:
          if (subtoken = strstr(token, tokenString)) {
        ...
        case NO: {
          ...
          if (subtoken = strstr(token, tokenCommentsStart)) {
            commentsLevel = MULTILINE;
          }
          if ((ptr = strstr(token, tokenCommentsLine)) &&
              (!subtoken || ptr < subtoken)) {
            commentsLevel = ENDOFLINE;
            subtoken = ptr;
          }
          if ((ptr = strstr(token, tokenString)) &&
              (!subtoken || ptr < subtoken)) {
    ...
    } while (token = STRTOK_CALL(nullptr, delimitors, &context));
    if (del.start != nullptr) {
      if (commentsLevel == ENDOFLINE)
        commentsLevel = NO;
    ...
    for (auto d = dels.crbegin(); d != dels.crend(); d++) {
      strncpy(d->start, d->end, strlen(d->end) + 1);
    }
    ...
    char *linePtr = line;
    for (size_t i = 0, len = strlen(line); i < len && line[i] == ' ';
         ++i, ++linePtr) {
    };
    ss << linePtr << (*linePtr ? "\n" : "");
  }
